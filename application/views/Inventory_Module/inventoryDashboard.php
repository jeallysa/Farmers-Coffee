                                    ...
                                    <table id="coffeein" class="table hover order-column" cellspacing="0" width="100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <?php
                                                    ...
                                                    $sql="SELECT distinct raw_coffee FROM raw_coffee where raw_activation = 1 ORDER BY 1";
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <?php
                                                $qcount1 = $this->db->query("SELECT DISTINCT raw_coffee FROM raw_coffee ORDER BY 1;");
                            ...
                            <table id="coffeeout" class="table hover order-column" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <?php
                                                $qcount2 = $this->db->query("SELECT DISTINCT raw_coffee FROM raw_coffee ORDER BY 1;");
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                if (isset($month_filt)){
                                                    $query = $this->db->query("SELECT DISTINCT raw_coffee FROM raw_coffee ORDER BY 1;");
                                                    foreach ($query->result() AS $row){
                                                  $totalout ="SELECT sum(a.totalin) AS totalout FROM (SELECT IFNULL(SUM(quantity), 0) as totalin FROM raw_coffee a JOIN trans_raw b JOIN inv_transact c JOIN contracted_po d JOIN contracted_client e ON a.raw_id = b.raw_coffeeid AND b.trans_id = c.trans_id AND c.po_client = d.contractPO_id AND d.client_id = e.client_id  where raw_coffee = '".$row->raw_coffee."' and type = 'OUT' and month(transact_date) = '".$month_filt."' UNION SELECT IFNULL(SUM(quantity), 0) as totalin FROM raw_coffee a JOIN trans_raw b ON a.raw_id = b.raw_coffeeid JOIN inv_transact c ON b.trans_id = c.trans_id JOIN walkin_sales d ON c.sales_inv = d.walkin_id  where raw_coffee = '".$row->raw_coffee."'  and type = 'OUT' and month(transact_date) = '".$month_filt."' UNION SELECT IFNULL(SUM(quantity), 0) as totalin FROM raw_coffee a JOIN trans_raw b ON a.raw_id = b.raw_coffeeid JOIN inv_transact c ON b.trans_id = c.trans_id JOIN company_returns d ON c.company_returnID = d.company_returnID where raw_coffee = '".$row->raw_coffee."'  and type = 'OUT' and month(transact_date) = '".$month_filt."') a;";
                                                  ...
                                                  foreach ($query8->result() as $object) {
                                                    ...
                                                    $query = $this->db->query("SELECT DISTINCT raw_coffee FROM raw_coffee ORDER BY 1;");
                                                    foreach ($query->result() AS $row){
                                                  $totalout ="SELECT sum(a.totalin) AS totalout FROM (SELECT IFNULL(SUM(quantity), 0) as totalin FROM raw_coffee a JOIN trans_raw b JOIN inv_transact c JOIN contracted_po d JOIN contracted_client e ON a.raw_id = b.raw_coffeeid AND b.trans_id = c.trans_id AND c.po_client = d.contractPO_id AND d.client_id = e.client_id  where raw_coffee = '".$row->raw_coffee."' and type = 'OUT' and month(transact_date) = month(now()) UNION SELECT IFNULL(SUM(quantity), 0) as totalin FROM raw_coffee a JOIN trans_raw b ON a.raw_id = b.raw_coffeeid JOIN inv_transact c ON b.trans_id = c.trans_id JOIN walkin_sales d ON c.sales_inv = d.walkin_id  where raw_coffee = '".$row->raw_coffee."'  and type = 'OUT' and month(transact_date) = month(now()) UNION SELECT IFNULL(SUM(quantity), 0) as totalin FROM raw_coffee a JOIN trans_raw b ON a.raw_id = b.raw_coffeeid JOIN inv_transact c ON b.trans_id = c.trans_id JOIN company_returns d ON c.company_returnID = d.company_returnID where raw_coffee = '".$row->raw_coffee."'  and type = 'OUT' and month(transact_date) = month(now())) a;" ;
